import unittest
import sys

#APPENDS SRC FOLDER TO PATH
sys.path.append('../')
sys.path.append('../src')

from users import User, Moderator
from content import Post, Reply

class TestReplySystem(unittest.TestCase):
    def setUp(self):
        # Initialize objects for testing
        # arrange
        self.post = Post(id = 1, author_id = 1,  date = "1999-01-01 12:04:16", title="Title", content="This is a test post.", image="data:image/jpeg;base64,")
        self.reply = Reply(id = 1, author_id = 1, date = "1999-01-01 12:04:18", parent_post_id = 1, content="This is a test comment.", image="data:image/jpeg;base64,")
        self.user = User(id=1, username="user1", password="pass1", email="email@email.com")

    # TESTING GETTER
    def test_reply_get_id(self):
        # act
        reply_id = self.reply.get_id()

        #assert
        self.assertIsInstance(reply_id, int)

    def test_reply_get_author_id(self):
        # act
        reply_author_id = self.reply.get_author_id()

        #assert
        self.assertIsInstance(reply_author_id, int)

    def test_reply_get_date(self):
        # act
        reply_date = self.reply.get_date()

        # assert
        self.assertIsInstance(reply_date, str)
    
    def test_reply_get_parent_post_id(self):
        # act 
        reply_get_parent_post = self.reply.get_parent_post_id()

        # assert
        self.assertIsInstance(reply_get_parent_post, int)

    def test_reply_get_title(self):
        # act
        reply_title = self.reply.get_title()

        # assert
        self.assertIsInstance(reply_title, str)

    def test_reply_get_content(self):
        # act
        reply_content = self.reply.get_content()

        # assert
        self.assertIsInstance(reply_content, str)

    def test_reply_get_image(self):
        # act
        reply_image = self.reply.get_image()

        # assert
        self.assertIsInstance(reply_image, str)

# TESTING SETTERS
    def test_reply_set_id(self):
        # arrange
        self.reply.set_id(2)

        # act
        this_reply_id = self.reply.get_id()

        # assert
        self.assertEqual(this_reply_id, 2)

    def test_reply_set_invalid_id(self):
        # arrange
        reply_Id = None

        # act
        with self.assertRaises(TypeError) as context:
            self.reply.set_id(reply_Id)

        # assert
        self.assertEqual("Tipo inválido de id. Ids devem ser inteiros." , str(context.exception))


    def test_reply_set_date(self):
        # arrange
        self.reply.set_date("1999-01-01 12:04:16")

        # act
        this_reply_date = self.reply.get_date()

        # assert
        self.assertEqual(this_reply_date, "1999-01-01 12:04:16")

    def test_reply_set_invalid_date(self):
        # arrange
        reply_date = None

        # act
        with self.assertRaises(TypeError) as context:
            self.reply.set_date(reply_date)

        # assert
        self.assertEqual("Tipo inválido para data. Ids devem ser Strings na forma 'YYYY-mm-dd hh:mm:ss'." , str(context.exception))

    def test_reply_publish(self):
        # arrange
        self.reply.publish()

        # act
        this_reply_date = self.reply.get_date()

        # assert
        self.assertIsInstance(this_reply_date, str)

    def test_reply_delete(self):
        #arrange
        self.reply.delete()

        #act
        this_reply_id = self.reply.get_id()

        #assert
        self.assertIsNone(this_reply_id)

    def test_reply_render(self):
        # arrange
        self.reply.publish()

        # act
        this_reply_render = self.reply.render()

        # assert
        self.assertIsInstance(this_reply_render, str)


if __name__ == '__main__':
    unittest.main()
